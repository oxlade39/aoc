...
use aoclib::{cartesian::{Point, Plane, Transform}, distance::{ManhattenDistance, Distance}};
...
impl Sensor {
    ...
    fn y_border_points(&self, y: i64) -> Option<(Point, Point)> {
        ...
        if abs_distance_to_y > max_axes_delta {
            ...
            Some((
                self.pos.transform(&t[0]),
                self.pos.transform(&t[1]),
            ))
        }
    }
}
...
impl FromStr for Map {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for line in s.lines() {
            ...
            let sensor_x: i64 = parts[2].split("=").nth(1).unwrap().replace(",", "").parse().unwrap();
            let sensor_y: i64 = parts[3].split("=").nth(1).unwrap().replace(":", "").parse().unwrap();
            let beacon_x: i64 = parts[8].split("=").nth(1).unwrap().replace(",", "").parse().unwrap();
            ...
            sensors.push(Sensor { pos: (sensor_x, sensor_y).into(), closest_beacon: (beacon_x, beacon_y).into() });
...
#[derive(Debug, PartialEq)]
    ...
    impl Default for ExclusionArea {
        fn default() -> Self {
            Self { exclusion_size: 0, exclusions: vec![] }
        }
    }
    ...
    impl ExclusionArea {
        fn exclude(&mut self, range: (i64, i64)) {
            ...
            for (excl_start, excl_end) in self.exclusions.iter() {
                let is_inside = (range_start >= *excl_start && range_start <= *excl_end) || (range_end >= *excl_start && range_end <= *excl_end) || (range_start <= *excl_start && range_end >=*excl_end);
                if is_inside {
...
mod tests {
    ...
    fn test_new_exclusion() {
        ...
        assert_eq!(exclusion_area, ExclusionArea {
            exclusion_size: 1,
            exclusions: vec![(1, 1)],
        })
    }
    ...
    fn test_exclusion_areas_2() {
        ...
        assert_eq!(existing_area, ExclusionArea {
            exclusions: vec![
                (0, 90)
            ],
            exclusion_size: 91

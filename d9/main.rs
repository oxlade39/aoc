...
fn part1() {
    ...
    let items: Vec<Vec<i64>> = input.lines()
        .map(|l| l.split("").filter_map(|n| n.parse::<i64>().ok()).collect::<Vec<_>>())
        .collect();
...
struct Postion{ row: usize, col: usize }

fn basin_size(width: usize, height: usize, position: Postion, grid: &Vec<Vec<i64>>, lows: &HashSet<Postion>) -> HashSet<Postion> {
    ...
    if row > 0 {
        let up = basin_size(width, height, Postion{ row: row - 1, col}, grid, &return_set);    
        return_set.extend(up);    
    }

    if col > 0 {
        let left = basin_size(width, height, Postion{ row, col: col - 1}, grid, &return_set);
        return_set.extend(left);
    }

    if row < height - 1 {
        let down = basin_size(width, height, Postion{ row: row + 1, col}, grid, &return_set);
        return_set.extend(down);
    }
    
    if col < width - 1 {
        let right = basin_size(width, height, Postion{ row, col: col + 1}, grid, &return_set);

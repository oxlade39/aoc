...
fn calc(i: &Instruction) -> i64 {
    match i {
        ...
        Instruction::Operator(_, InstructionType::Max, items) => {
            items.iter().map(|item| calc(item)).max().unwrap()
        },
        Instruction::Operator(_, InstructionType::Min, items) => {
            items.iter().map(|item| calc(item)).min().unwrap()
        },
        Instruction::Operator(_, InstructionType::Product, items) => {
            items.iter().map(|item| calc(item)).product()
        },
        Instruction::Operator(_, InstructionType::Sum, items) => {
            items.iter().map(|item| calc(item)).sum()
        }
    }
}
...
fn calc_examples() {
    ...
    let mut result: Vec<_> = instructions.iter().map(|item| calc(item)).collect();
    assert_eq!(result[0], 3);

    instructions = parse(&hex_to_bin("04005AC33890"));
    result = instructions.iter().map(|item| calc(item)).collect();
